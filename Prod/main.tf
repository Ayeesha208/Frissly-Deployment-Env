...
module "ecr" {
  ...
  repository_name = "prod-docker-repo"
  # vpc_id                  = module.vpc.vpc_id
  # public_subnet_id_value  = module.vpc.Public_subnet_id
...
module "app_runner_backend" {
  ...
  app_runner_service_name         = "Prod-Backend"
  repository_name                 = "prod-docker-repo" # Replace with your ECR repository name
  image_tag                       = "Prod"             # Replace with your specific tag if necessary
  image_repository_type           = "ECR"
  auto_scaling_configuration_name = "Prod-backend-autoscaling"
  role_name                       = "Prod-Apprunner-Role-1"
  policy_name                     = "Prod-Apprunner-Policy-1"
  max_concurrency                 = "100"
  ...
  private_subnet_id_value         = module.vpc.Private_subnet_id
  security_group_name             = "Prod_Apprunner_Security_group"
}
...
resource "null_resource" "name" {
  ...
  provisioner "file" {
    source      = "./module/ec2_instance/prod.sh"
    destination = "/home/ubuntu/prod.sh"
  }
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "rm /home/ubuntu/.env",
      "sudo chmod +x /home/ubuntu/prod.sh",
      "sh /home/ubuntu/prod.sh"
    ]
  }

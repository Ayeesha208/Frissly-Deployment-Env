...
module "ec2" {
  ...
  volume_size         = "30"
  volume_type         = "gp3"
  security_group_name = "Stagging-Ec2-Security-Group"
...
module "ecr" {
  ...
  repository_name = "stagging-docker-repo"
  # vpc_id                  = module.vpc.vpc_id
  # public_subnet_id_value  = module.vpc.Public_subnet_id
...
module "app_runner_backend" {
  ...
  app_runner_service_name         = "Stagging-Backend"
  repository_name                 = "stagging-docker-repo" # Replace with your ECR repository name
  image_tag                       = "Stagging"             # Replace with your specific tag if necessary
  image_repository_type           = "ECR"
  auto_scaling_configuration_name = "Stagging-backend-autoscaling"
  role_name                       = "Stagging-Apprunner-Role-1"
  policy_name                     = "Stagging-Apprunner-Policy-1"
  max_concurrency                 = "100"
  ...
  private_subnet_id_value         = module.vpc.Private_subnet_id
  security_group_name             = "Stagging_Apprunner_Security_group"
}
...
resource "null_resource" "name" {
  ...
  provisioner "file" {
    source      = "./module/ec2_instance/Stagging.sh"
    destination = "/home/ubuntu/Stagging.sh"
  }
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "rm /home/ubuntu/.env",
      "sudo chmod +x /home/ubuntu/Stagging.sh",
      "sh /home/ubuntu/Stagging.sh"
    ]
  }

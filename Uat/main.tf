...
module "ec2" {
  ...
  volume_size         = "30"
  volume_type         = "gp3"
  security_group_name = "UAT-Ec2-Security-Group"
...
module "ecr" {
  ...
  repository_name = "uat-docker-repo"
  # vpc_id                  = module.vpc.vpc_id
  # public_subnet_id_value  = module.vpc.Public_subnet_id
...
module "app_runner_backend" {
  ...
  app_runner_service_name         = "UAT-Backend"
  repository_name                 = "uat-docker-repo" # Replace with your ECR repository name
  image_tag                       = "uat"             # Replace with your specific tag if necessary
  image_repository_type           = "ECR"
  auto_scaling_configuration_name = "UAT-backend-autoscaling"
  role_name                       = "UAT-Apprunner-Role-1"
  policy_name                     = "UAT-Apprunner-Policy-1"
  max_concurrency                 = "100"
  ...
  private_subnet_id_value         = module.vpc.Private_subnet_id
  security_group_name             = "UAT_Apprunner_Security_group"
}
...
resource "null_resource" "name" {
  ...
  provisioner "file" {
    source      = "./module/ec2_instance/uat.sh"
    destination = "/home/ubuntu/uat.sh"
  }
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "rm /home/ubuntu/.env",
      "sudo chmod +x /home/ubuntu/uat.sh",
      "sh /home/ubuntu/uat.sh"
    ]
  }
